...
function parseEntitySlots(
  ...
  keccak.forEach((k) => {
    Object.values(stakeInfoEntities).forEach((info) => {
      ...
      if (entitySlots[addr] == null) entitySlots[addr] = new Set<string>();
      const currentEntitySlots = entitySlots[addr];
      if (k.startsWith(addrPadded)) currentEntitySlots.add(keccak256(k));
    });
  });
...
export function tracerResultParser(
  ...
  const illegalNonZeroValueCall = callStack.find(
    (call) => call.to !== entryPointAddress && !BigNumber.from(call.value ?? 0).eq(0),
  );
  requireCond(
