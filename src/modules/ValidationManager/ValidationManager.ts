...
import { calcPreVerificationGas } from '../../sdk/calcPreVerificationGas';
import {
...
export class ValidationManager {
  ...
  parseValidationResult(
    ...
    res: ValidationResultStructOutput,
  ): ValidationResult {
  ...
  async _callSimulateValidation(userOp: UserOperation): Promise<ValidationResult> {
    ...
    try {
      ...
      const [res] = entryPointSimulations.decodeFunctionResult(
        'simulateValidation',
        simulationResult,
      ) as ValidationResultStructOutput[];
  ...
  async _geth_traceCall_SimulateValidation(
    ...
    const provider = this.entryPoint.provider as JsonRpcProvider;
    const simulateCall = entryPointSimulations.encodeFunctionData('simulateValidation', [
      packUserOp(userOp),
    ]);
    ...
    if (lastResult.type === 'REVERT') {
      throw new RpcError(
        decodeRevertReason(data, false) as string,
  ...
  validateInputParameters(
    ...
    requireCond(
      BigNumber.from(userOp.preVerificationGas).gte(calcPreVerificationGas1),
